...
async function getFilteredExpenses<T>(
  model: Model<T>,
...
async function getGeneralMonthResume<T>(
  model: Model<T>,
  userName: string,
  startDate: Date,
  endDate: Date,
...
async function getVisibleAmount<T>(
  model: Model<T>,
  filteredMatch: {
    [p: string]: any;
...
export const paginateWithFilters = async <T extends { amount: number }>(
  ...
  const visibleAmount = filteredSummary.length > 0 ? filteredSummary[0].totalFilteredAmount : 0;
