...
import { ExpenseUpdateDto } from "@dtos/expense.dto";
...
export async function GET(_: NextRequest, { params }: Ctx) {
  try {
    ...
    const item = await getExpense(params.id, userId);
...
export async function PUT(req: NextRequest, { params }: Ctx) {
  try {
    ...
    const patch = ExpenseUpdateDto.parse(body);
    const updated = await updateExpense(params.id, userId, patch);
...
export async function DELETE(_: NextRequest, { params }: Ctx) {
  try {
    await connectDB();
    const userId = currentUserId();
    const res = await deleteExpense(params.id, userId);
