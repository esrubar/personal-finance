...
import {
  Button, Card, Col, Divider, InputNumber, Row,
  Select, Table, Typography, Space, Tag, message, Popconfirm
} from 'antd';
import {
  CopyOutlined,
  SaveOutlined,
...
*/
  ...
  const updateItem = (key: string, field: string, value: any) => {
    setPlanItems(prev => prev.map(item => item.key === key ? { ...item, [field]: value } : item));
  };
      ...
      render: (_: any, record: any) => (
          ...
          >
            {record.type === 'expense'
                ...
                : savingProjects?.map(p => <Option key={p._id} value={p._id}>{p.name}</Option>)
            }
          </Select>
      )
    },
        ...
        <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
            <Col>
              <Space size="middle">
                ...
                <Title level={2} style={{ margin: 0 }}>Planificador Mensual</Title>
              </Space>
            </Col>
            <Col>
              <Button
          ...
          <Row gutter={24} style={{ marginBottom: 24 }}>
            ...
            <Col span={4}>
              ...
              <InputNumber size="large" style={{ width: '100%', marginTop: 8 }} value={month} min={1} max={12} onChange={v => setMonth(v || 1)} />
            </Col>
            <Col span={4}>
              <Text type="secondary">Año</Text>
              <InputNumber size="large" style={{ width: '100%', marginTop: 8 }} value={year} onChange={v => setYear(v || 2026)} />
            </Col>
            <Col span={8}>
              <div style={{ background: '#f5f5f5', padding: '12px 20px', borderRadius: 8, textAlign: 'right' }}>
                <Text type="secondary">Disponible:</Text>
                <Title level={3} style={{ margin: 0, color: salary - totalAssigned < 0 ? '#ff4d4f' : '#52c41a' }}>
          ...
          <Space style={{ marginBottom: 16 }}>
            <Button type="primary" danger icon={<PlusCircleOutlined />} onClick={() => addItem('expense')}>
              Añadir Gasto
            </Button>
            <Button type="primary" style={{ background: '#52c41a', borderColor: '#52c41a' }} icon={<PlusCircleOutlined />} onClick={() => addItem('saving')}>
              Añadir Ahorro
            </Button>
          </Space>
          ...
          <Table
              ...
              summary={() => (
                  <Table.Summary.Row style={{ background: '#fafafa', fontWeight: 'bold' }}>
                    <Table.Summary.Cell index={0} colSpan={2}>Total Planificado</Table.Summary.Cell>
